...
echo "Sigel in $filename eintragen..."
  ...
  catmandu convert MARC to MARC --fix "marc_add('912', a, '$sigel1')" < "$file" >> ./output/"$filename"_sigel.mrc
  # Zweites Sigel
  if [ "$sigel2" != "n" ] ; then \
    catmandu convert MARC to MARC --fix "marc_add('912', a, '$sigel2')" < ./output/"$filename"_sigel.mrc >> ./output/"$filename"_sigel2.mrc
